#include "csapp.h"
/*Thanks to stkang9409*/

void doit(int fd);
void read_requesthdrs(rio_t *rp, int fd, char* gi);
...
//argc 와 argv -> 배열 길이, filename, port
int main(int argc, char **argv)
{
    int listenfd, connfd;                  //file description
...
// doit-트랜잭션 처리함수. 들어온 요청을 읽고 분석.
// GETrequest가 들어오면 정적인지 동적인지 파악하여서 각각에 맞는 함수를 실행
// 오류시 에러표시도 포함.
void doit(int fd)
{
    int is_static;
    ...
    // get인 경우 다른 요청 헤더 무시.
    read_requesthdrs(&rio, fd, gi);

    printf("$$$$$$$$$$$$$$$$$$$$$$$\n");
    printf("%s", gi);
    Rio_writen(fd, gi, strlen(gi));
    printf("$$$$$$$$$$$$$$$$$$$$$$$\n");
    // URI 분석하기
    // 파일이 없는 경우 에러 띄우기
...
void read_requesthdrs(rio_t *rp, int fd, char* gi)
{
    ...
    sprintf(gi, "%s %s",gi,  buf);
    while(strstr(buf,"end") == NULL)     // buf가 요청 헤더의 마지막이 되면 0 반환하고 끝.
    {
...
//동적인 파일을 받았을때 fork 함수로 자식프로세스를 만든후에 거기서 CGI프로그램 실행한다. s
void serve_dynamic(int fd, char *filename, char *cgiargs)
{
